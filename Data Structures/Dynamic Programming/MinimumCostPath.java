...
public class MinimumCostPath {
    public static void main(String[] args) {
        int[][] arr = {{9, 6, 0, 12, 90, 1},
                ...
                {9, 6, 2, -10, 7, 4},
                {10, -2, 0, 5, 5, 7}};
        System.out.println(minCostPathRecursive(arr));
        System.out.println(minCostPathDP(arr));
        System.out.println(minCostPathMemoization(arr));
    }

    public static int minCostPathMemoization(int[][] arr) {
        int m = arr.length;
        int n = arr[0].length;
        ...
        return minCostPathMemoizationHelper(arr, 0, 0, storage);
    }

    public static int minCostPathMemoizationHelper(int[][] arr, int i, int j, int[][] storage) {
        int m = arr.length;
        int n = arr[0].length;
        ...
        int op1 = minCostPathMemoizationHelper(arr, i, j + 1, storage);
        int op2 = minCostPathMemoizationHelper(arr, i + 1, j + 1, storage);
        int op3 = minCostPathMemoizationHelper(arr, i + 1, j, storage);
        storage[i][j] = arr[i][j] + Math.min(op1, Math.min(op2, op3));
        return storage[i][j];
    }

    //    Min-Cost Path from (i,j) cell to (m-1,n-1) cell
    public static int minCostPathRecursive(int[][] arr) {
        return minCostPathRecursiveHelper(arr, 0, 0);
    }

    public static int minCostPathRecursiveHelper(int[][] arr, int i, int j) {
        int m = arr.length;
        int n = arr[0].length;
        if (i == m - 1 && j == n - 1) {
            return arr[i][j];
        ...
        int op1 = minCostPathRecursiveHelper(arr, i, j + 1);
        int op2 = minCostPathRecursiveHelper(arr, i + 1, j + 1);
        int op3 = minCostPathRecursiveHelper(arr, i + 1, j);
