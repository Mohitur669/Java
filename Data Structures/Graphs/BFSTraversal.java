...
public class BFSTraversal {
        ...
        while (q.size() != 0) {
            ...
            for (int i = 0; i < edges.length; i++) {
                if (edges[firstElem][i] == 1 && !visited[i]) {
                    // check for all neighbours of firstElement if they have an edge and are not
                    // visited put them in the queue
                    q.add(i);
                    visited[i] = true;
                }
            }
    ...
    public static void main(String[] args) {
        ...
        int V = s.nextInt(); // V = Vertices
        int E = s.nextInt(); // E = Edges
