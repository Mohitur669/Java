...
Each of the following E lines contains two space separated integers,
    ...
    public static void helper(int[][] edges) {
        ...
        for (int i = 0; i < edges.length; i++) {
            if (!visited[i]) {
                ...
                helper2(edges, visited, arr, i); //here we get a coonected graph startting from i
                Collections.sort(arr); //here we are sorting the values in the arraylist by using collections

                for (int j = 0; j < arr.size(); j++) {
                    System.out.print(arr.get(j) + " ");
                }//after 1 connected graph is printed , we will give a nw line
                System.out.println();
            }
        }
    }

    public static void helper2(int[][] edges, boolean[] visited, ArrayList<Integer> arr, int start) {
